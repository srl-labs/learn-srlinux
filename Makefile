MKDOCS_VERSION ?= v9.7.5-3
MKDOCS_IMAGE := ghcr.io/eda-labs/mkdocs-material:$(MKDOCS_VERSION)
...
serve-docs-full:
	docker run -it --rm -p 8001:8000 -v $$(pwd):/docs --entrypoint mkdocs ${MKDOCS_IMAGE} serve -a 0.0.0.0:8000

# serve the doc site locally with dirty reload (faster, but toc updates may require a manual stop/start)
.PHONY: serve-docs
serve-docs:
	docker run -it --rm -p 8001:8000 -v $$(pwd):/docs ${MKDOCS_IMAGE} serve --dirtyreload -a 0.0.0.0:8000
...
build-insiders:
	docker run -v $$(pwd):/docs --entrypoint mkdocs ${MKDOCS_IMAGE} build --clean --strict

push-docs: # push docs to gh-pages branch manually. Use when pipeline misbehaves
	docker run -v ${SSH_AUTH_SOCK}:/ssh-agent --env SSH_AUTH_SOCK=/ssh-agent --env GIT_SSH_COMMAND="ssh -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no" -v $$(pwd):/docs --entrypoint mkdocs ${MKDOCS_IMAGE} gh-deploy --force --strict
